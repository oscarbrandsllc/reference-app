  ...
  teamChart = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        ...
        centerText: { text: team.totalValue.toFixed(0) },
      },
    },
  });
...
function parseSheetData(csv) {
  ...
  lines.forEach((line) => {
    ...
    const pos = clean(cols[2]);
    const sleeperId = clean(cols[12]);
    const ktc = parseInt(clean(cols[6]), 10);
    const age = parseFloat(clean(cols[3]));
    const adp = parseFloat(clean(cols[11]));
    const posRank = clean(cols[7]);
    if (pos === "RDP") {
      const pickName = clean(cols[1]);
      if (pickName) map[pickName] = { ktc: isNaN(ktc) ? 0 : ktc };
    } else if (sleeperId && sleeperId !== "NA") {
      map[sleeperId] = {
        ktc: isNaN(ktc) ? 0 : ktc,
        age: isNaN(age) ? null : age,
        adp: isNaN(adp) ? null : adp,
        posRank: posRank ? parseInt(posRank.split("·")[1], 10) : null,
      };
    }
  });
  return map;
