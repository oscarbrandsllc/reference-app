...
async function loadLeague(leagueId) {
  ...
  // sort by starter rank for consistent display
...
function buildCharts() {
  ...
  // vibrant unique color for each team, highlight user's team
  const starterData = teams.map((t) => t.starterTotal);
  const starterColors = teams.map((t, i) => {
    if (t.isUserTeam) return "var(--color-accent-primary)";
    const hue = Math.round((i / teams.length) * 360);
