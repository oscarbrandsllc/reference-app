...
let slotOrder = [];
let startersChart, totalChart, slotChart;
let currentTeams = [];
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('slotSelect').addEventListener('change', e => {
    ...
    if (slot) buildSlotChart(slot, currentTeams);
  });
});
...
async function loadLeague(leagueId) {
  ...
  currentTeams = teams;
  slotOrder = getSlotOrder(rosterPositions);
  computeSlotRanks(teams);
  buildCharts(teams);
  populateSlotTable(teams, slotOrder);
  populateSlotSelect(slotOrder);
  if (slotOrder.length) buildSlotChart(slotOrder[0], teams);

  document.getElementById('charts').classList.remove('hidden');
  document.getElementById('slotTableSection').classList.remove('hidden');
  document.getElementById('slotChartSection').classList.remove('hidden');
}
...
function getSlotOrder(rosterPositions) {
  const order = [];
  const counts = {};
  rosterPositions.forEach(slot => {
    if (['BN','IR','TAXI'].includes(slot)) return;
    counts[slot] = (counts[slot] || 0) + 1;
    const display = counts[slot] > 1 ? `${slot}${counts[slot]}` : slot;
    order.push(display);
  });
  return order;
}

function populateSlotSelect(order) {
...
function buildSlotChart(slot, teams) {
  const ctx = document.getElementById('slotChart').getContext('2d');
  const labels = teams.map(t => t.teamName);
  const data = teams.map(t => t.slotValues[slot] || 0);
  const base = slot.replace(/\d+/g, '');
  const colorMap = { QB: 'var(--pos-qb)', RB: 'var(--pos-rb)', WR: 'var(--pos-wr)', TE: 'var(--pos-te)', FLEX: 'var(--pos-wr)', SUPER_FLEX: 'var(--pos-qb)' };
  const playerNames = teams.map(t => t.starters.find(p => p.displaySlot === slot)?.name || '');
  const colors = teams.map(t => (t.isUserTeam ? 'var(--color-accent-primary)' : colorMap[base] || 'var(--pos-wr)'));
  if (slotChart) slotChart.destroy();
  slotChart = new Chart(ctx, {
    type: 'bar',
    data: { labels, datasets: [{ label: `${slot} KTC`, data, backgroundColor: colors, playerNames }] },
    options: {
      responsive: true,
      plugins: {
        ...
        tooltip: {
          callbacks: {
            afterLabel: ctx => {
              const name = ctx.dataset.playerNames[ctx.dataIndex];
              return name ? ` (${name})` : '';
            }
          }
        }
...
function buildCharts(teams) {
  ...
  const starterColors = teams.map(t=> t.isUserTeam ? 'var(--color-accent-primary)' : 'var(--pos-wr)');
  ...
  datasets.push({ label:'Picks', data: teams.map(t=>t.pickTotal), backgroundColor: colors.Picks, stack:'stack' });
...
function populateSlotTable(teams, slotOrder) {
  ...
  teams.forEach(team => {
    ...
    cells += slotOrder.map(slot => {
      ...
      return `<td title="${name}">${val.toFixed(0)}<br/><span class="rank">#${rank}</span></td>`;
    }).join('');
    row.innerHTML = cells;
