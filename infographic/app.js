...
function buildCharts() {
  ...
  startersChart = new Chart(ctx1, {
    ...
    options: {
      ...
      onClick: (evt) => {
        const points = startersChart.getElementsAtEventForMode(evt, 'nearest', { intersect: true }, true);
  ...
  totalChart = new Chart(ctx2, {
    ...
    options: {
      ...
      onClick: (evt) => {
        const points = totalChart.getElementsAtEventForMode(evt, 'nearest', { intersect: true }, true);
        if (points.length) selectTeam(points[0].index);
      }
    }
  });
...
function buildTeamChart(team) {
  ...
  const data = [team.posTotals.QB||0, team.posTotals.RB||0, team.posTotals.WR||0, team.posTotals.TE||0, team.pickTotal];
  const colors = ['var(--pos-qb)','var(--pos-rb)','var(--pos-wr)','var(--pos-te)','var(--pos-pick)'];
  ...
  document.getElementById('teamSummary').textContent = `Starters: ${team.starterTotal.toFixed(0)} | Total: ${team.totalValue.toFixed(0)}${ageText}`;
...
function populateSlotTable(rosterPositions) {
  ...
  teams.forEach((team,idx) => {
    ...
    cells += slotOrder.map(slot => {
      ...
      return `<td>${val.toFixed(0)}<br/><span class="rank">#${rank}</span></td>`;
    }).join('');
    row.innerHTML = cells;
...
function selectTeam(index) {
  ...
  const active = [];
  totalChart.data.datasets.forEach((_,dIdx) => active.push({datasetIndex:dIdx,index}));
