...
let startersChart, totalChart, teamChart, teamRadar;
...
function buildCharts() {
  ...
  const starterColors = teams.map((t) =>
    t.isUserTeam ? "var(--color-accent-primary)" : "var(--pos-wr)",
  );
        ...
        tooltip: {
          callbacks: {
            label: (ctx) => {
              ...
              return `${ctx.parsed.x.toFixed(0)} KTC (${rank})`;
            },
          },
        },
        datalabels: {
  ...
  const datasets = positions.map((pos) => ({
    ...
    borderColor: teams.map((t) =>
      t.isUserTeam ? "var(--color-accent-primary)" : "transparent",
    ),
    borderWidth: teams.map((t) => (t.isUserTeam ? 2 : 0)),
    stack: "stack",
    datalabels: {
      color: "#000",
      anchor: "center",
  ...
  datasets.push({
    ...
    datalabels: {
      ...
      formatter: (value, ctx) => {
        ...
        const rank = ordinalSuffix(teams[ctx.dataIndex].totalRank);
        return `${total.toFixed(0)} (${rank})`;
      },
    },
  });
        ...
        tooltip: {
          ...
          callbacks: {
            footer: (items) => {
              ...
              const rank = ordinalSuffix(teams[idx].totalRank);
              return `Total: ${teams[idx].totalValue.toFixed(0)} (${rank})`;
            },
          },
        },
      },
...
function buildTeamCharts(team) {
  ...
  const donutCtx = document.getElementById("teamChart").getContext("2d");
  if (teamChart) teamChart.destroy();
  teamChart = new Chart(donutCtx, {
    type: "doughnut",
    data: {
      labels,
      datasets: [{ data: donutData, backgroundColor: donutColors }],
    },
    options: {
      plugins: {
  ...
  const avgData = labels.map((pos, i) => {
    const sum = state.teams.reduce((s, t) => {
      return s + (pos === "Picks" ? t.pickTotal : t.posTotals[pos] || 0);
    }, 0);
    return sum / state.teams.length;
  });
  if (teamRadar) teamRadar.destroy();
  teamRadar = new Chart(radarCtx, {
    type: "radar",
    data: {
      labels,
      datasets: [
        {
          label: team.teamName,
          data: teamData,
          backgroundColor: "rgba(118, 109, 255, 0.2)",
        ...
        },
        {
          label: "League Avg",
          data: avgData,
          backgroundColor: "rgba(88, 167, 255, 0.2)",
          borderColor: "var(--pos-wr)",
          pointBackgroundColor: "var(--pos-wr)",
        },
      ],
    },
    options: {
...
function selectTeam(index) {
  ...
  buildTeamCharts(state.selectedTeam);
  // highlight bars
