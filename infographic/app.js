document.addEventListener('DOMContentLoaded', async () => {
    ...
    function calculatePositionalScores(teams) {
        ...
        teams.forEach(team => {
            for(let pos in maxes) {
                if(team.positionalScores[pos] > maxes[pos]) maxes[pos] = team.positionalScores[pos];
            }
        });
        ...
        teams.forEach(team => {
            for(let pos in maxes) {
                if(maxes[pos] > 0) {
                    team.positionalScores[pos] = (team.positionalScores[pos] / maxes[pos]) * 100;
                }
                leagueAverage[pos] += team.positionalScores[pos];
            }
        });
        for(let pos in leagueAverage) {
            leagueAverage[pos] /= teams.length;
        }
    ...
    function renderInfographic(teams, leagueInfo) {
        ...
        const sortedByStarter = [...teams].sort((a, b) => b.starterValue - a.starterValue);

        renderTeamTiers(sortedByTotal);
        renderBarChart('totalValue', totalValueChartCanvas, sortedByTotal, 'totalValue', 'Overall Team Value');
        renderBarChart('starterValue', starterValueChartCanvas, sortedByStarter, 'starterValue', 'Starter Value');
        ...
        state.charts[chartId] = new Chart(canvas, {
            ...
            options: {
                ...
                plugins: {
                    ...
                    tooltip: {
                        ...
                        external: context => {
                            ...
                            let innerHtml = `<h4>${team.teamName}</h4>`;
                            innerHtml += `<ul>
                ...
                scales: {
                    ...
                    y: { ticks: { color: '#EAEBF0' }, grid: { display: false } }
                }
            }
        });
    }
    ...
    function renderPositionalRanks(teams, leagueInfo) {
        ...
        for (const slot in lineupSlots) {
            ...
            const rankedTeams = teams.map(team => ({
                ...
                player: team.startersBySlot[slot] || { name: 'Empty', ktc: 0, pos: '' }
    ...
    function renderTeamDna(teams) {
        ...
        teams.forEach(team => {
            ...
            wrapper.className = 'radar-chart-wrapper glass-panel';
            const title = document.createElement('h3');
    ...
    // --- Helper Functions ---
    // (Implementations are the same as the previous correct version)
